...
/**
 * Creates a map from shape key name (e.g., 'Happy') to its index (e.g., 2)
 * for all morphable meshes in the scene.
 */
function buildShapeKeyMaps(rootGroup) {
    // Clear previous map
    for (const key in shapeKeyIndexMap) delete shapeKeyIndexMap[key]; 
    
    rootGroup.traverse(function (object) {
        if (object.isMesh && object.geometry && object.geometry.morphAttributes && object.geometry.morphAttributes.position) {
            // Check the standard Three.js property populated by most USD delegates
            const nameMap = object.userData.morphTargetDictionary || {};
            
            if (Object.keys(nameMap).length > 0) {
                shapeKeyIndexMap[object.name] = nameMap; 
                // console.log(`Mapped shape keys for ${object.name}:`, nameMap);
            } else {
                // console.warn(`Mesh ${object.name} has morph targets but no name dictionary found.`);
            }
        }
    });
    console.log("Shape Key Index Map Built:", shapeKeyIndexMap);
}

/**
 * Sets the influence for a specific shape key on a specific mesh.
 * This is the low-level function that applies the weight to the three.js array.
 * @param {string} meshName - Name of the Three.js mesh.
 * @param {string} shapeKeyName - Name of the blend shape.
 * @param {number} influence - Weight (0.0 to 1.0).
 */
function setShapeKeyInfluence(meshName, shapeKeyName, influence) {
    if (!shapeKeyIndexMap[meshName]) return; 

    const indexMap = shapeKeyIndexMap[meshName];
    const index = indexMap[shapeKeyName];
    
    if (index === undefined) {
        // console.warn(`Shape key '${shapeKeyName}' not found on mesh '${meshName}'.`);
        return; 
    }

    const targetMesh = window.usdRoot.getObjectByName(meshName);

    if (targetMesh && targetMesh.morphTargetInfluences) {
        targetMesh.morphTargetInfluences[index] = Math.min(1.0, Math.max(0.0, influence)); // Clamp 0-1
    }
}

/**
 * Iterates through the global state (window.activeEmotions) and applies all set influences.
 */
function applyActiveEmotions() {
    for (const meshName in window.activeEmotions) {
        const emotions = window.activeEmotions[meshName];
        for (const keyName in emotions) {
            // Use the low-level setter to apply the influence
            setShapeKeyInfluence(meshName, keyName, emotions[keyName]);
        }
    }
}
// ----------------------------------------------------------------------
